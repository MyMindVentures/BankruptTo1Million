...
function featuredPerson(point: JourneyPoint) {
  return {
    name: point.subject_name || point.author_name || personLabel(point.journey_person),
    avatarUrl: point.subject_avatar_url || point.author_avatar_url,
  };
}
...
function JourneyMap({ points, activeId, onSelect }: { points: JourneyPoint[]; activeId?: string; onSelect: (id: string) => void }) {
  ...
  return <div className="journey-map" aria-label={t('journal_journey_experience.map_label', 'Interactive journey map')}>
...
function HostOfferForm({ entry, onClose }: { entry: CalendarEntry; onClose: () => void }) {
  ...
  return <div className="journey-modal" role="dialog" aria-modal="true" aria-labelledby="host-offer-title"><div className="journey-modal__panel"><button className="journey-modal__close" type="button" onClick={onClose}>{t('journal_journey_experience.close', 'Close')}</button><p className="eyebrow">{t('journal_journey_experience.host_offer', 'Offer a place to stay')}</p><h3 id="host-offer-title">{t('journal_journey_experience.help_at', 'Help at {location}', { location: entry.location_name || entry.city_name || entry.title })}</h3><p>{t('journal_journey_experience.private_contact', 'Your contact details remain private and are only visible to authorized mission admins.')}</p>{state === 'success' ? <div className="journey-success"><HandHeart /><strong>{message}</strong><button className="button" onClick={onClose}>{t('journal_journey_experience.done', 'Done')}</button></div> : <form className="journey-host-form" onSubmit={submit}><div className="journey-form-grid"><label>{t('journal_journey_experience.name', 'Your name')}<input required value={form.host_name} onChange={(e) => setForm({ ...form, host_name: e.target.value })} /></label><label>{t('journal_journey_experience.email', 'Email')}<input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label>{t('journal_journey_experience.phone', 'Phone')} <small>{t('journal_journey_experience.optional', 'optional')}</small><input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label><label>{t('journal_journey_experience.accommodation_type', 'Accommodation type')}<input required placeholder={t('journal_journey_experience.accommodation_placeholder', 'Bed, camper place, guest room…')} value={form.accommodation_type} onChange={(e) => setForm({ ...form, accommodation_type: e.target.value })} /></label><label>{t('journal_journey_experience.available_from', 'Available from')}<input required type="date" value={form.available_from} onChange={(e) => setForm({ ...form, available_from: e.target.value })} /></label><label>{t('journal_journey_experience.available_until', 'Available until')}<input required type="date" value={form.available_until} onChange={(e) => setForm({ ...form, available_until: e.target.value })} /></label></div><label>{t('journal_journey_experience.message', 'Message')}<textarea required rows={4} placeholder={t('journal_journey_experience.message_placeholder', 'Tell Kevin and Micha what you can offer and anything useful to know.')} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} /></label><label className="journey-consent"><input required type="checkbox" checked={form.consent_to_contact} onChange={(e) => setForm({ ...form, consent_to_contact: e.target.checked })} /> {t('journal_journey_experience.contact_consent', 'Kevin and Micha may contact me about this hosting offer.')}</label>{message ? <p className={state === 'error' ? 'journey-error' : 'form-status'}>{message}</p> : null}<button className="button" disabled={state === 'saving'}>{state === 'saving' ? t('journal_journey_experience.sending', 'Sending privately…') : t('journal_journey_experience.send', 'Send hosting offer')}</button></form>}</div></div>;
}

export const JOURNAL_JOURNEY_EXPERIENCE_I18N_MANIFEST = {
  componentKey: 'components.journal.journey.experience',
  namespace: 'ui',
  translationKeys: [] as const,
  keyPatterns: ['journal_journey_experience.*'] as const,
...
export function JournalJourneyExperience() {
  ...
  if (loading) return <div className="journey-loading">{t('journal_journey_experience.loading', 'Loading the live route, plans and hosting opportunities…')}</div>;
  ...
  return <div className="journey-experience">
    ...
    <div className="journey-filters" role="group" aria-label={t('journal_journey_experience.filter', 'Filter journey by founder')}>{(['all', 'kevin', 'micha', 'together'] as const).map((value) => <button type="button" key={value} className={person === value ? 'is-active' : ''} onClick={() => setPerson(value)}>{value === 'all' ? t('journal_journey_experience.everyone', 'Everyone') : personLabel(value)}</button>)}</div>
    <div className="journey-map-layout"><JourneyMap points={filteredPoints} activeId={activePoint?.journey_entry_id} onSelect={setActiveId} /><aside className="journey-map-card">{activePoint ? <><div className="journey-map-card__header"><div className="journey-featured-person">{featuredPerson(activePoint).avatarUrl ? <img src={featuredPerson(activePoint).avatarUrl} alt={featuredPerson(activePoint).name} /> : <span>{featuredPerson(activePoint).name.charAt(0)}</span>}<div><small>{t('journal_journey_experience.featured_person', 'Featured person')}</small><strong>{featuredPerson(activePoint).name}</strong></div></div>{activePoint.is_current_location ? <span className="journey-live-badge">{t('journal_journey_experience.current_location', 'Current location')}</span> : null}</div>{activePoint.cover_image_url ? <img className="journey-map-card__image" src={activePoint.cover_image_url} alt="" /> : null}<p className="journey-map-card__date">{formatDate(activePoint.occurred_at)}</p><h4>{activePoint.title}</h4><p className="journey-map-card__location"><MapPin size={16} />{activePoint.map_label || activePoint.location_name || activePoint.city_name}{activePoint.country_name ? `, ${activePoint.country_name}` : ''}</p><p>{activePoint.what_happened || activePoint.excerpt}</p>{activePoint.why_it_mattered ? <div className="journey-map-card__insight"><strong>{t('journal_journey_experience.why_mattered', 'Why this mattered')}</strong><span>{activePoint.why_it_mattered}</span></div> : null}<div className="journey-map-card__meta">{activePoint.travel_mode ? <span>{activePoint.travel_mode}</span> : null}{activePoint.mood ? <span>{activePoint.mood}</span> : null}{activePoint.is_milestone ? <span>{t('journal_journey_experience.milestone', 'Milestone')}</span> : null}</div><a href={`/journal/${activePoint.slug}`}>{t('journal_journey_experience.read_chapter', 'Read this chapter')} <ChevronRight size={16} /></a></> : <><MapPin /><h4>{t('journal_journey_experience.first_pin', 'The first route pin is coming.')}</h4><p>{t('journal_journey_experience.first_pin_body', 'Publish a journey record with valid coordinates to activate this map.')}</p></>}</aside></div>
    <section className="journey-timeline-premium" aria-labelledby="real-journey-timeline"><div className="journey-subheading"><div><p className="eyebrow">{t('journal_journey_experience.interactive_timeline', 'Interactive timeline')}</p><h3 id="real-journey-timeline">{t('journal_journey_experience.timeline_title', 'From rock bottom, one real chapter at a time.')}</h3></div><Route /></div>{filteredPoints.length ? <div className="journey-timeline-track" ref={timelineTrackRef}>{filteredPoints.map((point) => { const featured = featuredPerson(point); return <button type="button" key={point.journey_entry_id} data-journey-id={point.journey_entry_id} className={activePoint?.journey_entry_id === point.journey_entry_id ? 'is-active' : ''} onClick={() => setActiveId(point.journey_entry_id)}><span className="journey-timeline-dot" /><div className="journey-timeline-person">{featured.avatarUrl ? <img src={featured.avatarUrl} alt={featured.name} /> : <span>{featured.name.charAt(0)}</span>}<b>{featured.name}</b></div><time>{formatDate(point.occurred_at)}</time><strong>{point.map_label || point.location_name || point.city_name || point.country_name || t('journal_journey_experience.update', 'Journey update')}</strong><small>{point.title}</small></button>; })}</div> : <div className="journey-empty-state">{t('journal_journey_experience.timeline_empty', 'Published journey stories will automatically build this timeline.')}</div>}</section>
