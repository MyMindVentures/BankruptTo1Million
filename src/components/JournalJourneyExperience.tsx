...
function JourneyMap({ points, activeId, onSelect }: { points: JourneyPoint[]; activeId?: string; onSelect: (id: string) => void }) {
  const positioned = useMemo(() => points.filter((point) => point.latitude != null && point.longitude != null), [points]);
  const bounds = useMemo(() => {
    const lats = positioned.map((point) => Number(point.latitude));
    const lngs = positioned.map((point) => Number(point.longitude));
    return {
      minLat: Math.min(...lats, 35), maxLat: Math.max(...lats, 45), minLng: Math.min(...lngs, -10), maxLng: Math.max(...lngs, 10),
    };
  }, [positioned]);
  const place = (point: JourneyPoint) => {
    const latRange = Math.max(bounds.maxLat - bounds.minLat, 1);
    const lngRange = Math.max(bounds.maxLng - bounds.minLng, 1);
    return {
      left: `${8 + ((Number(point.longitude) - bounds.minLng) / lngRange) * 84}%`,
      top: `${8 + (1 - (Number(point.latitude) - bounds.minLat) / latRange) * 74}%`,
    };
  };
  return <div className="journey-map" aria-label="Interactive journey map">
    ...
    {positioned.length > 1 ? <svg className="journey-map__route" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><polyline points={positioned.map((point) => { const p=place(point); return `${parseFloat(p.left)},${parseFloat(p.top)}`; }).join(' ')} /></svg> : null}
...
export function JournalJourneyExperience() {
  ...
  return <div className="journey-experience">
    <div className="journey-overview">
      <div><p className="eyebrow">Follow Kevin & Micha</p><h3>The journey, where it happened and where it goes next.</h3><p>Every pin connects to a published story. Future stops show where practical help, a bed or a safe camper place can turn a plan into the next chapter.</p></div>
      <div className="journey-stats"><span><strong>{status.locations_count}</strong> locations</span><span><strong>{status.stories_count}</strong> stories</span><span><strong>{Math.round(Number(status.total_distance_km || 0))}</strong> km</span><span><strong>{status.milestones_count}</strong> milestones</span></div>
    </div>
