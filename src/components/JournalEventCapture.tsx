...
function encodePlusCode(latitude: number, longitude: number, length = 10) {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    lat -= latDigit * placeValues[i];
    lng -= lngDigit * placeValues[i];
...
function LeafletPicker({ latitude, longitude, onPick }: { latitude: string; longitude: string; onPick: (lat: number, lng: number) => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const lat = Number(latitude) || 36.7213;
      const lng = Number(longitude) || -4.4214;
...
export function JournalEventCapture({ value, onChange, founders, people, eventTypes, files, onFilesChange, onPeopleRefresh, existingFootage = [], existingFootageHeading, showFootageOnlyUpload = false, onFootageOnlyUpload, footageOnlyUploadLabel, footageOnlyUploadHint, footageOnlyUploadDisabled = false, footageOnlyUploadBusy = false }: {
  ...
  const capturePhotoRef = useRef<HTMLInputElement | null>(null);
  const captureVideoRef = useRef<HTMLInputElement | null>(null);
  ...
  async function reverseLookup(lat: number, lng: number) {
    ...
    try {
      ...
      onChange({
        ...
        plus_code: encodePlusCode(lat, lng),
        location_name: result.name || result.address?.city || result.address?.town || result.address?.village || value.location_name,
        address_text: result.display_name || value.address_text,
      });
  ...
  async function createContact() {
    ...
    try {
      ...
      setContactForm({ display_name: '', full_name: '', email: '', phone: '', person_type: 'guest', location: '' });
    } finally {
  ...
  return <div className="event-capture-stack">
    <section className="event-panel event-panel-subject">
      <div className="event-panel-heading"><span>01</span><div><h3>Who is this story about?</h3><p>Select Kevin, Micha, or both.</p></div></div>
      <div className="founder-choice-grid">{founders.map((founder) => {
        const selected = value.subject_founder_ids.includes(founder.id);
        return <button type="button" key={founder.id} className={selected ? 'selected' : ''} onClick={() => onChange({ ...value, subject_founder_ids: selected ? value.subject_founder_ids.filter((id) => id !== founder.id) : [...value.subject_founder_ids, founder.id], journey_person: founder.slug.includes('kevin') ? 'kevin' : founder.slug.includes('micha') ? 'micha' : 'together' })}><div>{founder.label.slice(0, 1)}</div><span><strong>{founder.label}</strong><small>Founder journey</small></span>{selected && <Check size={17} />}</button>;
      })}</div>
    </section>

    <section className="event-panel">
      <div className="event-panel-heading"><span>02</span><div><h3>When and what happened?</h3><p>Define the event moment and story type.</p></div></div>
      <div className="event-two-column">
        <label>Event date and time<input type="datetime-local" required value={value.occurred_at} onChange={(e) => onChange({ ...value, occurred_at: e.target.value })} /></label>
        <label>Event type<select value={value.event_type} onChange={(e) => onChange({ ...value, event_type: e.target.value })}>{eventTypes.map((type) => <option key={type.key} value={type.key}>{type.label}</option>)}</select></label>
      </div>
    </section>
    ...
    <section className="event-panel"><div className="event-panel-heading"><span>05</span><div><h3>Capture or upload footage</h3><p>Photos and videos are uploaded to the Media Vault and linked to this journal post.</p></div></div>{existingFootage.length > 0 && existingFootageHeading && <div className="footage-existing"><p>{existingFootageHeading}</p><div className="footage-queue footage-queue--existing">{existingFootage.map((item) => {
      const url = publicStorageUrl(item.storage_bucket, item.storage_path);
