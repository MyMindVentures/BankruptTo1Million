...
function card(item: Opportunity, kind: 'partnership' | 'lead', index: number, t: WebsiteTranslate) {
  ...
  return `<article class="founder-opportunity-card"><div class="founder-opportunity-card__top"><span>${String(index + 1).padStart(2,'0')}</span><div><p class="eyebrow">${esc(kind === 'partnership' ? t('founder_opportunities.partnership', 'Partnership opportunity') : t('founder_opportunities.lead', 'Launch partner lead'))}</p><h3>${esc(item.name || item.organization || t('founder_opportunities.item', 'Opportunity'))}</h3></div></div><div class="founder-opportunity-card__meta">${item.organization && item.organization !== item.name ? `<span>${esc(item.organization)}</span>` : ''}${item.type ? `<span>${esc(item.type)}</span>` : ''}${item.role ? `<span>${esc(item.role)}</span>` : ''}${item.country ? `<span>${esc(item.country)}</span>` : ''}${item.status ? `<span>${esc(item.status)}</span>` : ''}${item.priority ? `<span>${esc(item.priority)} ${esc(t('founder_opportunities.priority', 'priority'))}</span>` : ''}${fit}</div>${item.summary || item.why_fit ? `<p>${esc(item.summary || item.why_fit)}</p>` : ''}${item.value ? `<div class="founder-opportunity-card__value"><strong>${esc(t('founder_opportunities.why_work', 'Why this could work'))}</strong><p>${esc(item.value)}</p></div>` : ''}${item.outreach_message ? `<details class="founder-opportunity-outreach"><summary>${esc(t('founder_opportunities.view_outreach', 'View outreach message'))}</summary><div>${item.outreach_subject ? `<strong>${esc(item.outreach_subject)}</strong>` : ''}<p>${esc(item.outreach_message)}</p>${outreachLink ? `<a class="button button--small" href="${esc(outreachLink)}">${esc(item.outreach_cta_label || t('founder_opportunities.open_outreach', 'Open outreach'))} →</a>` : ''}</div></details>` : ''}</article>`;
}
function render(payload: OpportunityPayload, slug: string, t: WebsiteTranslate) {
  ...
  section.innerHTML = `<div class="founder-opportunities__heading"><p class="eyebrow">${esc(t('founder_opportunities.eyebrow', 'Building the network'))}</p><h2>${esc(t('founder_opportunities.title', 'Best partnership and launch opportunities'))}</h2><p>${esc(t('founder_opportunities.description', 'Only the five strongest public matches are shown. Outreach stays collapsed until a visitor chooses to open it.'))}</p></div><div class="founder-opportunity-groups"><section><div class="founder-opportunity-group-title"><h3>${esc(t('founder_opportunities.partnerships', 'Top partnerships'))}</h3><span>${partnerships.length}/5</span></div>${partnerships.length ? `<div class="founder-opportunity-list">${partnerships.map((item,index)=>card(item,'partnership',index,t)).join('')}</div>` : `<div class="founder-opportunity-empty">${esc(t('founder_opportunities.empty_partnerships', 'No public partnership opportunities are published yet.'))}</div>`}</section><section><div class="founder-opportunity-group-title"><h3>${esc(t('founder_opportunities.leads', 'Top launch partner leads'))}</h3><span>${leads.length}/5</span></div>${leads.length ? `<div class="founder-opportunity-list">${leads.map((item,index)=>card(item,'lead',index,t)).join('')}</div>` : `<div class="founder-opportunity-empty">${esc(t('founder_opportunities.empty_leads', 'No public launch partner leads are published yet.'))}</div>`}</section></div>`;
  return section;
}

export const FOUNDER_POST_OPPORTUNITIES_UI_I18N_MANIFEST = {
  componentKey: 'lib.founder.post.opportunities.ui',
  namespace: 'ui',
  translationKeys: [] as const,
  keyPatterns: ['founder_opportunities.*'] as const,
  entityContent: { tables: [] },
} as const satisfies I18nManifest;

export function initializeFounderPostOpportunitiesUi(t: WebsiteTranslate) {
  let renderedSlug = '';
  let loadingSlug = '';
  const enhance = async () => {
    ...
    try {
      ...
      const section = render(payload,slug,t);
