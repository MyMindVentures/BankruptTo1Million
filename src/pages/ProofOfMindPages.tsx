...
function ConceptCard({ concept, onDiscovery }: { concept: ProofOfMindConcept; onDiscovery: (concept: ProofOfMindConcept) => void }) {
  ...
  return <article className={`concept-card proof-premium-card ${expanded ? 'concept-card--expanded' : 'concept-card--compact'}${concept.is_featured ? ' concept-card--featured' : ''}`}>
    ...
    {expanded ? <>
      ...
      <div className="proof-score-row"><ScoreBadge label="Concept" value={concept.concept_score} /><ScoreBadge label="Evaluation" value={concept.evaluation?.average_score} /><ScoreBadge label="Viral" value={concept.viral_score} /></div>
      {concept.evaluation?.criteria.length ? <div className="proof-all-scores"><div className="proof-all-scores__head"><span>All evaluation scores</span><strong>{concept.evaluation.criteria.length} criteria</strong></div><div className="proof-all-scores__grid">{concept.evaluation.criteria.map((item) => <article key={item.criterion}><span>{item.criterion}</span><strong>{item.score !== null ? `${item.score}/10` : '—'}</strong></article>)}</div></div> : null}
