...
const evaluationCriterionLabels: Record<string, string> = {
  'Kapitaalkrachtig doelpubliek': 'Affluent Target Audience',
  'Schaalbaarheid': 'Scalability',
  'Specifieke niche': 'Specific Niche',
  'Duidelijk klanttype: B2C, B2B of B2B2C': 'Clear Customer Type: B2C, B2B or B2B2C',
  'Urgentie en probleemdruk': 'Pain Urgency',
  'Terugkerende omzet': 'Recurring Revenue',
  'Marktomvang en groeipotentieel': 'Market Size and Growth Potential',
  'Duidelijk onderscheidend vermogen': 'Clear Differentiation',
  'Snel en betaalbaar te valideren': 'Fast and Affordable Validation',
  'Founder-market fit en uitvoerbaarheid': 'Founder–Market Fit and Execution Feasibility',
};

function getEvaluationCriterionLabel(label: string) {
  return evaluationCriterionLabels[label] || label;
}
...
function ConceptCard({ concept, onDiscovery }: { concept: ProofOfMindConcept; onDiscovery: (concept: ProofOfMindConcept) => void }) {
  ...
  return <article className={`concept-card proof-premium-card ${expanded ? 'concept-card--expanded' : 'concept-card--compact'}${concept.is_featured ? ' concept-card--featured' : ''}`}>
    ...
    {expanded ? <>
      ...
      <div className="proof-score-row"><ScoreBadge label="Concept Score" value={concept.concept_score} /><ScoreBadge label="Evaluation Score" value={concept.evaluation?.average_score} /><ScoreBadge label="Viral Score" value={concept.viral_score} /></div>
      {concept.evaluation?.criteria.length ? <div className="proof-all-scores"><div className="proof-all-scores__head"><span>All Evaluation Criteria</span><strong>{concept.evaluation.criteria.length} criteria</strong></div><div className="proof-all-scores__grid">{concept.evaluation.criteria.map((item) => <article key={item.criterion}><span>{getEvaluationCriterionLabel(item.criterion)}</span><strong>{item.score !== null ? `${item.score}/10` : '—'}</strong></article>)}</div></div> : null}
      ...
      <div className="concept-card__footer">{concept.competition.count ? <span>{concept.competition.count} competitors</span> : null}{leadSummary(concept) ? <span>{leadSummary(concept)}</span> : null}</div>
    </> : null}
