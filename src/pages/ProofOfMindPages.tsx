...
function ScoreBadge({ label, value }: { label: string; value: number | null | undefined }) { return value === null || value === undefined ? null : <div className="proof-score-chip"><strong>{value.toFixed(value % 1 ? 1 : 0)}/10</strong><span>{label}</span></div>; }
...
function ConceptCard({ concept, onDiscovery }: { concept: ProofOfMindConcept; onDiscovery: (concept: ProofOfMindConcept) => void }) {
  ...
  return <article className={`concept-card${concept.is_featured ? ' concept-card--featured' : ''}`}>
    ...
    <div className="proof-score-row"><ScoreBadge label="Concept score" value={concept.concept_score} /><ScoreBadge label="Evaluation score" value={concept.evaluation?.average_score} /></div>
    ...
    {concept.competition.competitive_advantage || concept.innovation_summary ? <div className="concept-card__innovation"><span>Competitive advantage</span><p>{concept.competition.competitive_advantage || concept.innovation_summary}</p></div> : null}
    ...
    <div className="concept-card__footer"><span>{concept.is_featured ? 'Featured' : 'Portfolio concept'}</span><span>{openable ? 'Full public page' : 'Protected teaser'}</span>{concept.primary_market ? <span>{concept.primary_market}</span> : null}{concept.competition.count ? <span>{concept.competition.count} competitors</span> : null}{leadSummary(concept) ? <span>{leadSummary(concept)}</span> : null}</div>
...
export function ProofOfMindDetailPage({ slug }: { slug: string }) {
  ...
  return <main id="top" className="proof-page proof-detail-page">
    <section className="proof-detail-hero section-grid" aria-labelledby="concept-title">
      <div className="proof-detail-hero__copy"><p className="eyebrow">Proof of Mind · Fully Public Concept</p><h1 id="concept-title">{concept.title}</h1>{concept.tagline ? <p className="hero__lede">{concept.tagline}</p> : null}{renderText(concept.short_description)}<div className="concept-card__meta"><span>{humanize(concept.concept_type)}</span>{concept.concept_format ? <span>{concept.concept_format}</span> : null}<span>{concept.category}</span><span>{humanize(concept.concept_status)}</span>{concept.primary_market ? <span>{concept.primary_market}</span> : null}{concept.delivery_model ? <span>{concept.delivery_model}</span> : null}{concept.physical_location_required !== null ? <span>{concept.physical_location_required ? 'Location-based' : 'Location-flexible'}</span> : null}</div><div className="proof-score-row"><ScoreBadge label="Concept score" value={concept.concept_score} /><ScoreBadge label="Evaluation score" value={concept.evaluation?.average_score} /></div>{concept.founder ? <p className="concept-card__founder">Created by {concept.founder.name}{concept.founder.is_original_creator ? ' · original creator' : ''}</p> : null}<div className="hero__actions">{primaryCta ? <a className="button" href={primaryCta} target="_blank" rel="noreferrer">{concept.detail_cta_label || 'Become a Launch Partner'} <ExternalLink size={16} /></a> : <a className="button" href="/support">Discuss the Concept</a>}{mediaLinks.map((link) => <a key={link.label} className="button button--ghost" href={link.url!} target="_blank" rel="noreferrer">{link.label}</a>)}</div></div>
    ...
    <div className="section concept-detail concept-detail--premium">
      <DetailBlock title="Concept overview">{concept.full_description || concept.vision_statement || concept.innovation_summary || concept.target_audience || concept.concept_format || concept.delivery_model || concept.primary_market ? <>{renderText(concept.full_description)}{renderText(concept.vision_statement)}{renderText(concept.innovation_summary)}{renderText(concept.target_audience)}<div className="concept-card__tags">{[concept.concept_format, concept.delivery_model, concept.primary_market].filter(Boolean).map((item) => <span key={item!}>{item}</span>)}</div></> : null}</DetailBlock>
      <DetailBlock title="Founder">{concept.founder ? <div className="proof-founder"><h3>{concept.founder.name}</h3>{concept.founder.role ? <p>{concept.founder.role}</p> : null}{concept.founder.is_original_creator ? <span>Original creator</span> : null}{renderText(concept.founder.bio)}</div> : null}</DetailBlock>
      <DetailBlock title="Commercial evaluation">{concept.evaluation ? <><div className="proof-score-row"><ScoreBadge label="Average score" value={concept.evaluation.average_score} /></div><div className="proof-evaluation-list">{concept.evaluation.criteria.map((item) => <article key={item.criterion}><h3>{item.criterion}</h3>{item.score !== null ? <strong>{item.score}/10</strong> : null}{renderText(item.assessment)}<DetailList items={item.risks} /><DetailList items={item.improvement_actions} /></article>)}</div></> : null}</DetailBlock>
      <DetailBlock title="Competition and positioning">{concept.competition.summary || concept.competition.comparisons.length || concept.competition.competitive_advantage ? <>{renderText(concept.competition.summary)}{concept.competition.competitive_advantage ? <div className="proof-advantage"><strong>Why we are different</strong><p>{concept.competition.competitive_advantage}</p></div> : null}<div className="proof-comparison-grid">{concept.competition.comparisons.slice(0, 10).map((item) => <article key={item.name}><h3>{item.name}</h3>{item.product ? <p>{item.product}</p> : null}{item.similarities ? <p><strong>Similarities:</strong> {item.similarities}</p> : null}{item.differences ? <p><strong>Differences:</strong> {item.differences}</p> : null}{item.our_advantage ? <p><strong>Our advantage:</strong> {item.our_advantage}</p> : null}{item.competitor_advantage ? <p><strong>Competitor advantage:</strong> {item.competitor_advantage}</p> : null}{item.strategic_risk ? <p><strong>Strategic risk:</strong> {item.strategic_risk}</p> : null}</article>)}</div></> : null}</DetailBlock>
      ...
      <DetailBlock title="Market and business model">{concept.market_opportunity || concept.tags.length ? <>{renderText(concept.market_opportunity)}<div className="concept-card__tags">{concept.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></> : null}</DetailBlock>
      ...
      <DetailBlock title="Partner and lead opportunities">{concept.lead_pipeline ? <><p>{concept.lead_pipeline.category_count} lead categories · {concept.lead_pipeline.identified_leads} identified leads · {concept.lead_pipeline.target_slots} target slots</p><div className="proof-lead-grid">{concept.lead_pipeline.categories.map((item) => <article key={item.name}><h3>{item.name}</h3>{item.strategic_goal ? <p>{item.strategic_goal}</p> : null}{item.default_outreach_angle ? <p><strong>Outreach angle:</strong> {item.default_outreach_angle}</p> : null}<span>{item.identified_leads} / {item.target_slots} identified</span>{item.outreach_progress ? <p>{item.outreach_progress}</p> : null}</article>)}</div></> : null}</DetailBlock>
      <DetailBlock title="Collaboration CTA">{concept.collaboration_opportunities.length || primaryCta ? <>{<DetailList items={concept.collaboration_opportunities} />}{primaryCta ? <a className="button" href={primaryCta} target="_blank" rel="noreferrer">{concept.detail_cta_label || 'Explore partnership'} <ExternalLink size={16} /></a> : <a className="button" href="/support">Join the mission</a>}</> : null}</DetailBlock>
