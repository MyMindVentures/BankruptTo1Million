...
export function ProofOfMindDetailPage({ slug }: { slug: string }) {
  ...
  return <main className="proof-page">
    ...
    <section className="section"><div className="container concept-detail">
      ...
      <DetailSection title="Competition and differentiation"><div className="proof-advantage"><strong>Why we are different</strong>{renderText(concept.competition.competitive_advantage || concept.innovation_summary)}</div>{concept.competition.comparisons.length ? <div className="proof-comparison-grid">{concept.competition.comparisons.map((competitor) => <article key={competitor.name}><h3>{competitor.name}</h3>{competitor.product ? <p className="proof-kicker">{competitor.product}</p> : null}{renderText(competitor.notes)}<DetailList items={competitor.differentiators} /></article>)}</div> : null}</DetailSection>
      {concept.evaluation ? <DetailSection title="Evaluation"><div className="proof-evaluation-list">{concept.evaluation.criteria.map((criterion) => <article key={criterion.criterion}><div><h3>{criterion.criterion}</h3><strong>{criterion.score !== null ? `${criterion.score}/10` : '—'}</strong></div>{renderText(criterion.reasoning)}</article>)}</div></DetailSection> : null}
      {concept.lead_pipeline ? <DetailSection title="Collaboration opportunities"><div className="proof-lead-grid">{concept.lead_pipeline.categories.map((category) => <article key={category.category}><h3>{category.category}</h3>{renderText(category.description)}<DetailList items={category.target_profiles} /></article>)}</div></DetailSection> : null}
      {pitchUrl ? <DetailSection title="Pitch deck"><a className="button" href={pitchUrl} target="_blank" rel="noreferrer">Open pitch deck <ExternalLink size={16} /></a></DetailSection> : null}
      <section className="concept-detail-section proof-signature"><Sparkles /><div><strong>Proof of Mind</strong><p>This concept is part of the public Bankrupt to 1 Million venture archive.</p></div></section>
      <section className="concept-detail-section proof-final-cta"><h2>Help turn this concept into reality</h2><p>Join as a customer, launch partner, investor, developer, sponsor or media partner.</p><button className="button" type="button" onClick={() => setDiscoveryOpen(true)}>Register interest</button></section>
