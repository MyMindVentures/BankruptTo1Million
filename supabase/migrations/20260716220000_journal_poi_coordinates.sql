-- POI coordinates for journal place context map view

alter table public.journal_post_pois
  add column if not exists latitude numeric,
  add column if not exists longitude numeric,
  add column if not exists google_maps_url text,
  add column if not exists coordinate_source text
    check (coordinate_source is null or coordinate_source in ('ai', 'geocoded', 'manual'));
...
-- Backfill demo post POI coordinates (Vélez-Málaga landmarks)
do $$
declare
  v_post_id uuid;
begin
