...
begin
  ...
  insert into public.media_assets(
    ...
    'public',
    'published',
    coalesce(asset_metadata,'{}'::jsonb) || jsonb_build_object('journal_post_id',post_id,'source','journal_event_capture'),
  ...
  if coalesce(display_index,0)=0 and kind='image' then
    ...
    set cover_image_url = concat('/storage/v1/object/public/',bucket_name,'/',object_path), updated_at=now()
    where id=post_id and cover_image_url is null;
  end if;
