-- Restore Media Vault post cards for journal posts with zero footage.
-- Merges all-posts LEFT JOIN (overwritten by captured_at) with captured_at category fields.
...
begin
  ...
  with posts as (
    select
      ...
      count(ma.id)::integer as asset_count,
      greatest(
        jp.updated_at,
        coalesce(max(jpm.updated_at), jp.updated_at)
      ) as updated_at
    from public.journal_posts jp
    left join public.journal_post_media jpm on jpm.journal_post_id = jp.id
    left join public.media_assets ma
      on ma.id = jpm.media_asset_id
     and ma.asset_type in ('image', 'video')
    group by jp.id, jp.title, jp.slug, jp.status, jp.updated_at
  ),
  cover_ranked as (
    select
  ...
  into v_posts
  from posts p
  left join cover_ranked c on c.post_id = p.post_id and c.rn = 1
  left join event_ranked e on e.post_id = p.post_id and e.rn = 1;
