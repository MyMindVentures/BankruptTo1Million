-- Use relative storage URLs in public_journal_map_points so the frontend can prefix VITE_SUPABASE_URL.
...
left join lateral (
  select jsonb_agg(
    jsonb_build_object(
      ...
      'url', coalesce(
        ma.external_url,
        '/storage/v1/object/public/' || ma.storage_bucket || '/' || ma.storage_path
      ),
